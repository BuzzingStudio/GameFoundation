    ...
    public class DictionaryGenericConverter: DefaultTypeConverter
    {
        ...
        public override string ConvertToString(object value, Type typeInfo)
        {
            if (value != null)
            {
                ...
                {
                    list.Add(string.Join(this.delimiterPair, new string[] { keyConverter.ConvertToString(itemData.Key, keyType), valueConverter.ConvertToString(itemData.Value, valueType) }));
                }
        ...
        public override object ConvertFromString(string text, Type typeInfo)
        {
            ...
            if (!string.IsNullOrEmpty(text))
            {
                ...
                foreach (var itemRawData in itemsRawData)
                {
                    ...
                    dictionary.Add(keyConverter.ConvertFromString(itemData[0], keyType),valueConverter.ConvertFromString(itemData[1], valueType));
                }
            }
