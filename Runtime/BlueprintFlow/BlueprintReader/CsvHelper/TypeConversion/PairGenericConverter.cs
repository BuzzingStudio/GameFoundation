    ...
    public class PairGenericConverter : DefaultTypeConverter
    {
        ...
        public override object ConvertFromString(string text, Type typeInfo)
        {
            if (!string.IsNullOrEmpty(text))
            {
                ...
                var tuple = (ITuple)Activator.CreateInstance(keyPairType, keyConverter.ConvertFromString(itemsRawData[0], keyType), valueConverter.ConvertFromString(itemsRawData[1], valueType));
                return tuple;
            }
