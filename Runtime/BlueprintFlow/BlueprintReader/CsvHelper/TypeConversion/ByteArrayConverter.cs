...
namespace GameFoundation.BlueprintFlow.BlueprintReader.CsvHelper.TypeConversion
{
	using System;
	using System.Text;
	...
	public class ByteArrayConverter : DefaultTypeConverter
	{
		...
		public ByteArrayConverter(ByteArrayConverterOptions options = ByteArrayConverterOptions.Hexadecimal | ByteArrayConverterOptions.HexInclude0x)
		{
			...
			this.HexStringPrefix = (options & ByteArrayConverterOptions.HexDashes) == ByteArrayConverterOptions.HexDashes ? "-" : string.Empty;
			this.ByteLength      = (options & ByteArrayConverterOptions.HexDashes) == ByteArrayConverterOptions.HexDashes ? (byte)3 : (byte)2;
		}
		...
		private byte[] HexStringToByteArray(string hex)
		{
			...
			{
				byteArray[(stringIndex - has0xOffset) / this.ByteLength] = Convert.ToByte(hex.Substring(stringIndex, 2), 16);
			}
		...
		private void ValidateOptions()
		{
			if ((this.options & ByteArrayConverterOptions.Base64) == ByteArrayConverterOptions.Base64)
			{
				...
				{
					throw new  ArgumentException($"{nameof(ByteArrayConverter)} must be configured exclusively with HexDecimal options, or exclusively with Base64 options.  Was {this.options.ToString()}")
					{
