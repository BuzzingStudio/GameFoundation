    ...
    public class Tuple3GenericConverter : DefaultTypeConverter
    {
        ...
        public override object ConvertFromString(string text, Type typeInfo)
        {
            if (!string.IsNullOrEmpty(text))
            {
                ...
                var tuple = (ITuple)Activator.CreateInstance(keyPairType, item1Converter.ConvertFromString(itemsRawData[0], item1Type),
                    item2Converter.ConvertFromString(itemsRawData[1], item2Type), item3Converter.ConvertFromString(itemsRawData[2], item3Type));
                return tuple;
            }
        ...
        public override string ConvertToString(object value, Type typeInfo)
        {
            ...
            var item2Converter = CsvHelper.TypeConverterCache.GetConverter(item2Type);
            var item3Converter = CsvHelper.TypeConverterCache.GetConverter(item3Type);
            return string.Join(this.delimiterPair, item1Converter.ConvertToString(keyPairType.GetProperty("Item1").GetValue(value),item1Type),
