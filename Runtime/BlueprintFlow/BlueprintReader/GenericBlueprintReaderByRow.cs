namespace GameFoundation.BlueprintFlow.BlueprintReader
{
    ...
    public abstract class GenericBlueprintReaderByRow<T1, T2> : BlueprintByRow<T1, T2>, IGenericBlueprintReader where T2 : class
    {
    ...
    public class BlueprintRecordReader<TRecord> where TRecord : class
    {
        ...
        private void Setup()
        {
            var csvHeaderKeyAttribute = (CsvHeaderKeyAttribute)Attribute.GetCustomAttribute(this.blueprintType, typeof(CsvHeaderKeyAttribute));
        ...
        public TRecord GetRecord(CsvDataReader inputCsv)
        {
            ...
            if (!string.IsNullOrEmpty(inputCsv.GetField(this.RequireKey)))
            {
                ...
                foreach (var memberInfo in this.fieldAndProperties)
                {
                    ...
                    {
                        throw new FieldDontExistInBlueprint($"{this.blueprintType.FullName} - {memberInfo.MemberName}- {e}");
                    }
                    catch (Exception e)
                    {
        ...
        public List<List<string>> ToRawData(TRecord inputObject, bool containHeader = false)
        {
            ...
            if (containHeader)
            {
                result.Add(this.fieldAndProperties.Select(memberInfo => memberInfo.MemberName).ToList());
            }
        ...
        private bool IsBlueprintCollection(Type type) { return (type.IsGenericType || type.BaseType is { IsGenericType: true }) && typeof(IBlueprintCollection).IsAssignableFrom(type); }
    }
}