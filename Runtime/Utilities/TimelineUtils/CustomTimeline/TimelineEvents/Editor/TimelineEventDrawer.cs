#if UNITY_EDITOR
namespace GameFoundation.Utilities.TimelineUtils.CustomTimeline.TimelineEvents.Editor {
    using System;
    ...
    public class TimelineEventDrawer : PropertyDrawer {
        ...
        public override void OnGUI(Rect position, SerializedProperty property, GUIContent label) {
            ...
            selectedMethod = this.AddMethodsPopup("Method", handlerKeyProperty,
                clip.TrackTargetObject,
                isMethodWithParamProperty.boolValue);

            if (selectedMethod != null && isMethodWithParamProperty.boolValue) {
                var isSpecialType = this.AddEnumValuePopup(selectedMethod, argValueProperty) ||
                                    this.AddBoolValuePopup(selectedMethod, argValueProperty);
                if (!isSpecialType) {
                    this.AddGeneralValueInput(selectedMethod, argValueProperty);
                }
            }
            else {
        ...
        private MethodInfo AddMethodsPopup(string label, SerializedProperty property, GameObject gameObject,
            ...
            string[] callbackMethods   = this._eventHandlerListStart.Concat(callbackMethodsEnumarable).ToArray();
