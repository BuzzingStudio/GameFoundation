namespace GameFoundation.Utilities.TimelineUtils.Extensions {
    using System;
    using System.Collections.Generic;
    using System.Linq;
    using GameFoundation.Utilities.TimelineUtils.CustomTimeline.Slider;
    using GameFoundation.Utilities.TimelineUtils.CustomTimeline.TMPUITextCounter;
    using UnityEngine;
    ...
    public static class PlayableDirectorExtensions {
        ...
        public static void Bind(this PlayableDirector playableDirector, List<int> trackIndices,
        ...
        /// </summary>
        public static void Play(this PlayableDirector playableDirector, double startTime, double endTime,
            bool force = false, bool pauseOrStop = true) {
            playableDirector.time = startTime;
            if (force)
                playableDirector.Evaluate();
            var helper = playableDirector.GetComponent<PlayableDirectorHelper>();
            if (helper == null)
                helper = playableDirector.gameObject.AddComponent<PlayableDirectorHelper>();
            var tmp = new List<Action<double>>(helper.RegisteredActions);
            foreach (var action in tmp) {
                helper.OnUpdate -= action;
            }
            ...
            if (!playableDirector.gameObject.activeInHierarchy)
                Debug.LogWarning($"Playable director {playableDirector.gameObject.name} is not active");
            playableDirector.Play();
        }

        public static void PlayTimeScale(this PlayableDirector playableDirector, float timeScale,
            double startTime = -1d, double endTime = -1d) {
        ...
        /// <param name="markerTrackName">Name of the track <seealso cref="Marker.MarkerTrack"/></param>
        /// <param name="markerClipName">Name of the clip <seealso cref="MarkerClip"/></param>
        public static void JumpTo(this PlayableDirector playableDirector, string markerTrackName,
            ...
            if (tmp > 1) {
                ...
                Debug.LogWarning($"Clip {markerClipName} of track {markerTrackName} not found ???");
            }
        ...
        public static void JumpTo(this PlayableDirector playableDirector, string markerClipName) {
            ...
            if (tmp > 1) {
                ...
                Debug.LogWarning($"Clip {markerClipName} not found ???");
            }
        ...
        public static void AddOnceCompleteListener(this PlayableDirector playableDirector, Action callback) {
            if (onCompleteCallback.TryGetValue(playableDirector.GetHashCode(), out var listCallback)) {
                ...
                onCompleteCallback.Add(playableDirector.GetHashCode(),new List<Action>{callback});
                playableDirector.stopped += PlayableDirectorOnStopped;
            }
        }

        private static Dictionary<int,List<Action>> onCompleteCallback = new Dictionary<int,List<Action>>();
        private static void PlayableDirectorOnStopped(PlayableDirector obj) {
            if (onCompleteCallback.TryGetValue(obj.GetHashCode(),out var listCallback)) {
                foreach (var callback in listCallback) {
                    callback?.Invoke();
                }
                listCallback.Clear();
            }
