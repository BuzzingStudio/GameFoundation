namespace MechSharingCode.Utils
{
    ...
    public static class ListExtensions
    {
        ...
        public static (List<T> passes, List<T> fails) SplitOnPredicate<T>(this IEnumerable<T> list, Func<T, bool> predicate)
        {
        ...
        public static IEnumerable<T> EveryNthElement<T>(this IEnumerable<T> list, int n) => list.Where((x, i) => i % n == 0);

        public static IEnumerable<Type> Types<T>(this IEnumerable<T> set) where T : notnull => set.Select(x=>x.GetType());
...
#nullable disable
        public static (bool, T) AllSame<T>(this IEnumerable<T> set)
        {
            ...
            if (!enumerable.Any()) return (false, default(T));
...
#nullable enable
        ...
        public static T GetOrAdd<T>(this IList<T> list, Func<T, bool> match) where T : new() => GetOrAdd(list, match, () => new T());
        ...
        public static T Random<T>(this IList<T> list, Random random) => list[random.Next(0, list.Count)];
    ...
    public static class ArrayExtensions
    {
        ...
        public static IEnumerable<T> CurveOrder<T>(this T[] listToOrder)
        {
            ...
            for (int i = 0; i < listToOrder.Length; i += 2) yield return listToOrder[i];

            // Move backward every other two. Note: Length%2 makes sure we're on the correct offset.
            for (int i = listToOrder.Length - 1 - (listToOrder.Length % 2); i >= 0; i -= 2) yield return listToOrder[i];
        }
    ...
    public static class QueueExtensions
    {
        ...
        /// <summary>
        /// Optimized non-alloc version of <see cref="AddRange{T}(System.Collections.Generic.Queue{T},System.Collections.Generic.IEnumerable{T})"/>.
        /// Because <see cref="List{T}.GetEnumerator"/> returns struct <see cref="List{T}.Enumerator"/>, but <see cref="IEnumerable{T}.GetEnumerator"/> returns <see cref="IEnumerator{T}"/> as boxed version of <see cref="List{T}.Enumerator"/>.
        /// </summary>
    ...
    public static class CollectionExtensions
    {
        ...
        public static bool ContainsAll<T>(this ICollection<T> source, IEnumerable<T> values) => values.All(source.Contains);

        /// <summary> Checks if <paramref name="source"/> collection contains any value from <paramref name="values"/>. </summary>
        public static bool ContainsAny<T>(this ICollection<T> source, IEnumerable<T> values) => values.Any(source.Contains);

         public static T    IndexOrDefault<T>(this ICollection<T> list, int index)               => (list.HasIndex(index) ? list.ElementAt(index) : default(T))!;
         public static T    IndexOrDefault<T>(this ICollection<T> list, int index, T defaultVal) => list.HasIndex(index) ? list.ElementAt(index) : defaultVal;
        public static  bool HasIndex<T>(this       ICollection<T> list, int index) => !(index < 0 || index >= list.Count);
    ...
    public static class ArrayUtil
    {
        ...
        public static T SecondLast<T>(this T[] array) => array[array.Length - 1];
    ...
    public static class ListUtil
    {
        ...
        public static IEnumerable<TEntry> TakeSame<TEntry, TVal>(this IEnumerable<TEntry> list, Func<TEntry, TVal> testSame) where TVal : IComparable
        {
        ...
        public static IEnumerable<(T Node, int Level)> DepthFirstTraversal<T>(this T root, Func<T, IEnumerable<T>> getChildren)
        {
    ...
    public static class SetExtensions
    {
        ...
        public static IEnumerable<TValue> LimitGroups<TKey, TValue> (this IEnumerable<IGrouping<TKey, TValue>> groups, Func<TValue, int> getCountAllowed)
        {
